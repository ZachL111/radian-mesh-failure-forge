...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-mesh-failure-detail.ps1

sqlite3 :memory: ".read tests/domain_review.sql"

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-mesh-failure-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-mesh-failure-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian_mesh_failure_forge-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
