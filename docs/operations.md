...
`radian-mesh-failure-forge` is centered on distributed systems work in SQL. The technical target is: Implement an SQL distributed systems project for failure resource planning, using capacity fixtures and allocation and spill reports.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
